...
class MenuController
{
    ...
    public function getDataCreate(Request $request)
    {
        ...
        if ($type) {
            ...
            if ($type == MenuItem::TYPE_CATEGORY_POST) {
    ...
    public function searchData(Request $request)
    {
        ...
        if ($request->menu_type == MenuItem::TYPE_CATEGORY_POST) {
            $data = Taxon::where('name', 'LIKE', $request->query('q').'%')
                ->where('taxonomy_id', setting('post_taxonomy', 1))
            ...
            $data->getCollection()->transform(function ($personal) {
                ...
                $prettyName = '';
                if (@$personal->ancestors->isNotEmpty()) {
                    foreach (@$personal->ancestors as $ancestor) {
                        $prettyName .= $ancestor->name.' -> ';
                    }
                }
                $prettyName .= @$personal->name;
