...
@section('title', __('Chỉnh sửa :model', ['model' => $package->name]))
 

@section('page-content')
    @include('admin.packages._form', [
        'url' =>  route('admin.packages.update', $package),
        'package' => $package ?? new \App\Domain\Package\Models\Package,
        'method' => 'PUT'
    ])
@stop
@push('css')
...
@push('js')
    ...
    <script>
        ...
        function setTaxonSelect(selector) {
            ...
            if ($(selector).length > 0) {
                $(selector).select2({
                    ...
                    width: '100%',
                    ajax: {
                        url: "{{ route('admin.taxons.search', ['type' => 'package_taxonomy']) }}",
                        datatype: 'json',
                        delay: 250,
        ...
        Dropzone.options.packageImages = {
            url: '{{ route('admin.packages.upload.image') }}',
            maxFilesize: 2,
            ...
            success: function (file, response) {
                $('#package-form').append('<input type="hidden" name="new_images[]" value="' + response.file + '">')
                file.previewElement.classList.add("dz-success")
            },
            removedfile: function (file) {
                file.previewElement.remove()
                if (file.hasOwnProperty('id')) {
                    $('#package-form').find('input[name="images[]"][value="' + file.id + '"]').remove()
                } else {
                    $('#package-form').find('input[name="new_images[]"][value="' + file.name + '"]').remove()
                }
            },
            init: function () {
                var myDropzone = this;
                this.on("addedfile", function(file) {
                    ...
                    @if($package->getMedia()->isNotEmpty())
                    @foreach($package->getMedia() as $key => $image)
                        ...
                        $('#package-form').append('<input type="hidden" name="images[]" value="{{ $image->id }}">');
                    @endforeach
                    @endif
                let fileCountOnServer = '{{ $package->getMedia()->count() }}';
    ...
    {!! JsValidator::formRequest('App\Http\Requests\Admin\PackageUpdateRequest', '#package-form'); !!}
@endpush
