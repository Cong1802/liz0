...
// Home > Packages
Breadcrumbs::for('admin.packages.index', function (BreadcrumbsGenerator $trail) {
    $trail->parent('admin.dashboard');
    $trail->push(__('Gói tin'), route('admin.packages.index'), ['icon' => 'fal fa-edit']);
});

// Home > Packages > Create

Breadcrumbs::for('admin.packages.create', function (BreadcrumbsGenerator $trail) {
    $trail->parent('admin.packages.index');
    $trail->push(__('Tạo'), route('admin.packages.create'));
});

// Home > Packages > [admin] > Edit
Breadcrumbs::for('admin.packages.edit', function (BreadcrumbsGenerator $trail, Package $package) {
    $trail->parent('admin.packages.index');
    $trail->push($package->name, '#');
    $trail->push(__('Chỉnh sửa'), route('admin.packages.edit', $package));
});
/*
|--------------------------------------------------------------------------
| System Breadcrumbs
